...
template <typename T>
T CritRelSimplexDiameter(
  const std::vector<std::vector<T>>& S,
  const std::vector<int>& s,
  const T& epsilon,
  ska::unordered_map<std::vector<int>, int, VectorHasher>& CritID,
  std::vector<std::vector<int> >& E
    ...
    if (d > 2) {
        if (s[d - 1] >= S.size()) {
            ...
            for (const auto& tau : subsets) {
                std::vector<int> sigma;
                sigma = E[CritID[{tau[0],tau[1]}]];
                if (sigma[1] < S.size()) {
                    ...
                    temp = T{0.5} * epsilon - vectorDistance(S[0].begin(), S[0].end(), S[sigma[0]].begin());
                    if (temp > T{0}) {
                        maxsofar = temp;
                    } else {
                        maxsofar = T{0};
        ...
        std::vector<int> sigma;
        sigma = E[CritID[{s[0],s[1]}]];
        if (sigma[1] < S.size()) {
            maxsofar = T{0.5} * vectorDistance(S[sigma[0]].begin(), S[sigma[0]].end(), S[sigma[1]].begin());
        } else {
            temp = T{0.5} * epsilon - vectorDistance(S[0].begin(), S[0].end(), S[sigma[0]].begin());
            if (temp > T{0}) {
                maxsofar = temp;
  ...
  std::vector<std::vector<int> >& E
    ...
    if (d > 1) {
        for (int i = 0; i < d; i++) {
            for (int j = i + 1; j < d; j++) {
                // Compute midpoint (m)
                std::vector<int> t = E[CritID[{s[i],s[j]}]];
                auto first = S[t[0]].begin();
                auto first2 = S[t[1]].begin();
                ...
                if (vectorDistance(m.begin(), m.end(), S[0].begin()) < T{0.5} * epsilon) {
                    temp = BoundaryProjectedDistance(S[t[0]].begin(), S[t[0]].end(), S[t[1]].begin(), S[0].begin(), S[0].end(), T{0.5} * epsilon);
                } else {
                    temp = T{0.5} * vectorDistance(S[t[0]].begin(), S[t[0]].end(), S[t[1]].begin());
                }
...
void CritAddRelCofaces(
    ...
    int d = t.size();
    V[d-1].push_back({CritRelSimplexDiameter(S,t,epsilon,CritID,E),t});
            ...
            if(f < S.size())
                {
                    ...
                    G[f].begin(), G[f].end(),
                    std::back_inserter(M));
                    CritAddRelCofaces(S, G, k, epsilon, s, M, V, CritID, E);
                }
            else
            {
                std::vector<int> M;
...
void CritBndAddCofaces(
    ...
    int d = t.size();
    V[d - 1].push_back({CritBndSimplexDiameter(S, t, epsilon, CritID, E), t});
    ...
    if (d < k + 2) {
        for (const int& f : N) {
            if (std::find(t.begin(), t.end(), f) == t.end()) {
                ...
                CritBndAddCofaces(S, G, k, epsilon, s, M, V, CritID, E);
            }
        }
    }
    return;
...
template <typename T>
Cplx<T> CritRelIncrementalVR(
    const std::vector<std::vector<T>>& S,
    const int& k,
    const T& epsilon,
    const std::list<std::vector<std::vector<int>>>& cplx_nbh,
    ska::unordered_map<std::vector<int>, int, VectorHasher>& CritID,
    std::vector<std::vector<int> >& E
) {
    Cplx<T> VR(k+2);
    VR.neighborhood = cplx_nbh;
    ...
    for(int i = 0; i<n; i++)
    {
        if((VR.neighborhood).front()[i].size() > 0)
        {
            u[0] = (VR.neighborhood).front()[i][0];
            CritAddRelCofaces(
            S, (VR.neighborhood).front(),
            k, epsilon, u, 
            (VR.neighborhood).front()[u[0]], VR.simplices, CritID, E
            );
        }
    }
